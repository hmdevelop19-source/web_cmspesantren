...
import { 
  ...
  ArrowRight, LayoutDashboard, Heart, Menu, Search, X as CloseIcon, Calendar, Megaphone, Newspaper, Loader2, AlertCircle
} from 'lucide-react';
...
export default function PublicLayout() {
  ...
  return (
    <div className="flex flex-col min-h-screen font-sans">
      <header className={`w-full fixed top-0 z-50 transition-all duration-500 ease-in-out ${isTransparent ? 'bg-transparent py-4' : 'bg-white/90 backdrop-blur-xl shadow-lg shadow-gray-200/20 py-6 border-b border-gray-100/50'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center">
            
            {/* Logo Section */}
            <Link to="/" className="flex items-center gap-4 group">
              <div className={`transition-all duration-500 flex items-center justify-center rounded-2xl ${isTransparent ? 'w-14 h-14 bg-secondary/10' : 'w-12 h-12 bg-primary/5'} p-2 group-hover:scale-110 group-hover:rotate-3 shadow-2xl shadow-black/5 ring-1 ring-white/20 whitespace-nowrap`}>
            ...
            <nav className="hidden lg:flex items-center space-x-1">
              <Link to="/" className={`px-4 py-4 text-[11px] font-black uppercase tracking-[0.1em] transition-all ${isActive('/')}`}>Beranda</Link>
              <Link to="/profil" className={`px-4 py-4 text-[11px] font-black uppercase tracking-[0.1em] transition-all ${isActive('/profil')}`}>Profil</Link>
              <Link to="/berita" className={`px-4 py-4 text-[11px] font-black uppercase tracking-[0.1em] transition-all ${isActive('/berita')}`}>Berita</Link>
              <Link to="/kontak" className={`px-4 py-4 text-[11px] font-black uppercase tracking-[0.1em] transition-all ${isActive('/kontak')}`}>Kontak</Link>
              ...
              <Link to="/login" className="text-[11px] font-black text-primary-dark bg-secondary hover:bg-yellow-400 px-6 py-3.5 rounded-xl transition-all uppercase tracking-widest shadow-lg shadow-secondary/20 active:scale-95 ml-2">
                Portal Admin
              </Link>
            </nav>
      ...
      <div className={`fixed inset-0 z-[60] lg:hidden transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`absolute top-0 right-0 w-4/5 max-w-sm h-full bg-white shadow-2xl transition-transform duration-300 ease-out flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
           ...
           <div className="p-6 flex flex-col gap-2 flex-grow overflow-y-auto">
              ...
              <Link to="/profil" onClick={() => setIsMobileMenuOpen(false)} className={`px-5 py-4 rounded-2xl text-xs font-black uppercase tracking-widest ${location.pathname === '/profil' ? 'bg-primary text-white' : 'bg-gray-50 text-gray-600'}`}>Profil</Link>
              <Link to="/berita" onClick={() => setIsMobileMenuOpen(false)} className={`px-5 py-4 rounded-2xl text-xs font-black uppercase tracking-widest ${location.pathname === '/berita' ? 'bg-primary text-white' : 'bg-gray-50 text-gray-600'}`}>Warta / Berita</Link>
              <Link to="/kontak" onClick={() => setIsMobileMenuOpen(false)} className={`px-5 py-4 rounded-2xl text-xs font-black uppercase tracking-widest ${location.pathname === '/kontak' ? 'bg-primary text-white' : 'bg-gray-50 text-gray-600'}`}>Hubungi Kami</Link>
           </div>
      ...
      <main className={`flex-grow bg-white ${isHomepage ? '' : 'pt-24'}`}>
        <Outlet />
      </main>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-16 mb-20">
            ...
            <div className="md:col-span-3">
              ...
              <ul className="space-y-5 text-[13px] text-white/40 font-black uppercase tracking-widest italic font-mono">
                ...
                <li>
                  <Link to="/profil" className="hover:text-secondary transition-all flex items-center gap-3 group">
                    <ArrowRight className="w-3.5 h-3.5 opacity-0 -ml-6 group-hover:opacity-100 group-hover:ml-0 transition-all duration-300 text-secondary" />
                    Profil
                  </Link>
                </li>
                <li>
                  <Link to="/berita" className="hover:text-secondary transition-all flex items-center gap-3 group">
                    <ArrowRight className="w-3.5 h-3.5 opacity-0 -ml-6 group-hover:opacity-100 group-hover:ml-0 transition-all duration-300 text-secondary" />
                    Berita
                  </Link>
                </li>
                <li>
                  <Link to="/kontak" className="hover:text-secondary transition-all flex items-center gap-3 group">
                    <ArrowRight className="w-3.5 h-3.5 opacity-0 -ml-6 group-hover:opacity-100 group-hover:ml-0 transition-all duration-300 text-secondary" />
                    Kontak
                  </Link>
                </li>
            ...
            <div className="md:col-span-4">
              ...
              <ul className="space-y-8">
                <li className="flex gap-6 group">
                  ...
                  <div>
                    <span className="text-[9px] text-white/30 font-black uppercase tracking-[0.2em] block mb-1.5 font-mono">CAMPUS LOCATION</span>
                ...
                <li className="flex gap-6 items-center group">
                  ...
                  <div>
                    <span className="text-[9px] text-white/30 font-black uppercase tracking-[0.2em] block mb-1.5 font-mono">OFFICIAL MAIL</span>
          ...
          <div className="border-t border-white/5 pt-12 mt-12 flex flex-col lg:flex-row justify-between items-center gap-8">
            ...
            <div className="flex items-center gap-10">
                ...
                <div className="flex items-center gap-2 text-[10px] font-black text-white/10 uppercase tracking-widest group">
                    Made with <Heart className="w-3 h-3 text-red-900/50 fill-red-900/30 group-hover:text-red-600 group-hover:fill-red-500 transition-all duration-700" /> by Antigravity
                </div>
            </div>
          </div>
        </div>
