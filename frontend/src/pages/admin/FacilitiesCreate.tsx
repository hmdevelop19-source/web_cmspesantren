import { ArrowLeft, Save, Building2, ImageIcon, LayoutList, Type, Info } from 'lucide-react';
...
export default function FacilitiesCreate() {
  ...
  const mutation = useMutation({
    ...
    onError: (error: any) => {
      alert('Gagal menambah fasilitas: ' + (error.response?.data?.message || 'Terjadi kesalahan'));
    },
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex items-center justify-between border-b border-slate-200 pb-8 text-left">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Tambah Fasilitas</h1>
            <p className="text-sm text-slate-500 mt-1">Daftarkan sarana atau prasarana baru pesantren</p>
          </div>
        </div>
          ...
          <div className="bg-primary/5 p-6 rounded-3xl border border-primary/10 border-dashed">
             <div className="flex gap-4">
                ...
                <p className="text-[11px] text-slate-500 leading-relaxed font-medium">
                  <strong>Tips:</strong> Gunakan foto berkualitas tinggi dan deskripsi yang jelas untuk memberikan kesan terbaik bagi calon wali santri.
                </p>
             </div>
          </div>
        </div>
